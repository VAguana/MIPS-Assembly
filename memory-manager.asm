...
init:
	...
	#Guardamos la posicion donde empieza la memoria:
	sw $v0, heapHead
	
	
	# syscall allocate. Creamos la refList del mismo tamaño de init_size (ya está en a0)
	li  $v0, 9
	syscall 
...
malloc:
	...
	lw $s0, init_size    # Cargamos el valor de init size en t0
	lw $s1, ini_bloq
	lw $s2, fin_bloq
